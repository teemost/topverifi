...
<head>
    ...
    <title>Admin Sign In — TopVerifi</title>
...
<div class="min-h-screen grid md:grid-cols-2">
    {{-- Left panel --}}
    <div class="hidden md:flex flex-col justify-between p-12 bg-gradient-to-br from-slate-900 via-slate-800 to-orange-950">
        <div class="flex items-center gap-2">
            ...
            <span class="font-bold text-white">Top<span class="text-brand">Verifi</span> Admin</span>
        </div>
        <div>
            ...
            <form method="POST" action="{{ route('admin.login.post') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" required autocomplete="username"
                        class="w-full bg-slate-900 border border-slate-600 rounded-lg px-4 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:border-brand focus:ring-1 focus:ring-brand">
                </div>
                <div>
                ...
                <button type="submit"
                    class="w-full bg-brand hover:bg-brand-dark text-white font-semibold py-2.5 px-4 rounded-lg transition-colors mt-2">
                    Sign in
                </button>
            </form>

            <div class="mt-6 text-center text-sm text-slate-400">
                <a href="/" class="text-brand hover:underline">← Back to site</a>
            </div>
        </div>
    </div>
</div>
