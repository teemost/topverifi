...
<head>
    ...
    <title>@yield('title', 'Admin') — TopVerifi</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <script>
        tailwind.config = {
            theme: { extend: { colors: { brand: { DEFAULT: '#f97316', dark: '#ea580c' } } } }
        }
    </script>
    <style>
        *, *::before, *::after { box-sizing: border-box; }
        ...
        input[type=text],input[type=email],input[type=number],input[type=password],select,textarea {
            ...
            font-size:max(16px, .875rem);outline:none;
        }
        input:focus,select:focus,textarea:focus { border-color:#f97316; }
        .btn-primary { background:#f97316;color:#fff;padding:.5rem 1.25rem;border-radius:.5rem;font-size:.875rem;font-weight:500;cursor:pointer;border:none;transition:background .15s; }
...
<div id="admin-overlay" onclick="closeMobileSidebar()" class="fixed inset-0 z-30 bg-black/60 backdrop-blur-sm hidden lg:hidden"></div>

<aside id="admin-sidebar" class="w-64 bg-slate-800 border-r border-slate-700 flex flex-col min-h-screen fixed top-0 left-0 bottom-0 z-40 -translate-x-full lg:translate-x-0">
    ...
    <nav class="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        ...
        <a href="{{ route('admin.boosting-orders') }}" class="sidebar-link {{ request()->routeIs('admin.boosting-orders*') ? 'active' : '' }}">
        ...
        <a href="{{ route('admin.bank-transfers') }}" class="sidebar-link {{ request()->routeIs('admin.bank-transfers*') ? 'active' : '' }}">
            ...
            <span id="bt-badge" class="ml-auto flex items-center gap-1">
                @if($userConfirmedPending > 0)
                    <span id="bt-paid-badge" class="bg-red-500 text-white text-xs font-bold px-1.5 py-0.5 rounded-full">{{ $userConfirmedPending }}</span>
                @else
                    <span id="bt-paid-badge" class="hidden bg-red-500 text-white text-xs font-bold px-1.5 py-0.5 rounded-full"></span>
                @endif
        ...
        @php $errorCount = \App\Models\PurchaseErrorLog::whereDate('created_at', today())->count(); @endphp
        <a href="{{ route('admin.purchase-errors') }}" class="sidebar-link {{ request()->routeIs('admin.purchase-errors*') ? 'active' : '' }}">
            ...
            Purchase Errors
            @if($errorCount > 0)
                <span class="ml-auto bg-red-500 text-white text-xs font-bold px-1.5 py-0.5 rounded-full">{{ $errorCount }}</span>
            @endif
        </a>
...
<div class="lg:ml-64 flex-1 flex flex-col min-h-screen min-w-0 overflow-x-hidden">
    <header class="bg-slate-800 border-b border-slate-700 px-4 lg:px-8 py-4 flex items-center justify-between sticky top-0 z-30">
        <div class="flex items-center gap-3">
            ...
            <h1 class="hidden lg:block text-base lg:text-lg font-semibold text-white">@yield('page-title', 'Dashboard')</h1>
        </div>
        <div class="flex items-center gap-2 lg:gap-3">
            <a href="{{ route('home') }}" target="_blank" class="hidden sm:block text-xs text-slate-400 hover:text-brand transition-colors">View Site →</a>
