...
<head>
    ...
    <title>@yield('title', 'Dashboard') — TopVerifi</title>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .sidebar-link.active svg { color:#f97316; }

        #dash-sidebar { transition: transform 0.28s cubic-bezier(.4,0,.2,1); }
        ...
        [data-theme="light"] .sidebar-link.active { background:rgba(249,115,22,0.1) !important; color:#ea580c !important; }
...
<div id="dash-overlay" onclick="closeMobileSidebar()" class="fixed inset-0 z-30 bg-black/60 backdrop-blur-sm hidden lg:hidden"></div>
...
<aside id="dash-sidebar" class="w-60 bg-slate-800 border-r border-slate-700 flex flex-col min-h-screen fixed top-0 left-0 bottom-0 z-40 -translate-x-full lg:translate-x-0">
    ...
    <nav class="flex-1 px-3 py-4 space-y-0.5">
        ...
        <a href="{{ route('dashboard.boosting-orders') }}" class="sidebar-link {{ request()->routeIs('dashboard.boosting-orders*') ? 'active' : '' }}">
        ...
        <a href="{{ route('dashboard.api-keys') }}" class="sidebar-link {{ request()->routeIs('dashboard.api-keys*') ? 'active' : '' }}">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"/></svg>
            API Access
        </a>
        <a href="{{ route('dashboard.api-guide') }}" class="sidebar-link {{ request()->routeIs('dashboard.api-guide*') ? 'active' : '' }}">
...
<div class="lg:ml-60 flex-1 flex flex-col min-h-screen min-w-0 overflow-x-hidden">
    <header class="bg-slate-800 border-b border-slate-700 px-4 lg:px-8 py-4 flex items-center justify-between">
        ...
        <div class="flex items-center gap-3 lg:gap-4 text-sm text-slate-400">
            ...
            <a href="{{ route('dashboard.wallet') }}" class="text-brand hover:text-orange-300 text-xs">Top up →</a>
        </div>
    </header>
